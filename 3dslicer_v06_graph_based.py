...
def _check_mp4_progress(viz_dir: Path, expected_files: list[tuple[str, str]]) -> dict:
    ...
    """
    results = {"total": len(expected_files), "created": 0, "missing": [], "files": {}}
        ...
        for size, target_label in size_label_pairs:
            ...
            for ep_z, ep_y, ep_x in target_endpoints:
                ...
                candidate_indices = global_tree.query_ball_point(
                    ep_coord_scaled, r=max_distance_voxels
                )
                ...
                distances = np.linalg.norm(
                    valid_coords_scaled - ep_coord_scaled[None, :], axis=1
                )
...
def _preprocess_vessel_data_with_island_connection(
    ...
    if viz_dir:
        try:
            _visualize_intermediate_stage(
                connected_skeleton.astype(np.float32),
                viz_dir,
    ...
    print(
        f"  Dilated skeleton: {dilated_voxels} voxels ({100*dilated_voxels/total_voxels:.4f}% of volume)"
    )

    if dilated_voxels == 0:
        print("  ⚠️  WARNING: Dilated skeleton is empty - preprocessing may have failed")
    elif dilated_voxels < MIN_DILATED_VOXELS_WARNING:
...
def _visualize_3d_vessels_from_numpy(vessel_data: np.ndarray, viz_dir: Path) -> None:
    ...
    print(
        f"Vessel voxels: {vessel_voxels} ({100*vessel_voxels/vessel_data.size:.2f}% of volume)"
    )
    ...
    try:
        ...
        try:
            ...
            if (
                "x server" in error_msg
                or "display" in error_msg
                or "bad x server connection" in error_msg
            ):
                print("\n" + "=" * 60)
                ...
                print(
                    "       xvfb-run -s '-screen 0 1920x1080x24' python 3dslicer_v06_graph_based.py ..."
                )
        ...
        print(
            "  Using volume rendering with linear opacity and 'Blues' colormap (notebook style)"
        )
        ...
        try:
            print(
                f"  Generating {VIZ_N_FRAMES} frame rotating video (framerate={VIZ_FRAMERATE})..."
            )
            ...
            azimuth_angles = (
                180 + np.arange(n_frames, dtype=np.float32) * 360.0 / n_frames
            )
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                ...
                print(
                    f"✓ 3D vessel visualization saved: {output_path} ({file_size:.2f} MB)"
                )
            else:
                print("⚠ Warning: MP4 file created but appears empty or missing")
        except (SystemExit, KeyboardInterrupt):
            raise
        except Exception as movie_error:
            print(
                f"⚠ Warning: Movie generation failed (continuing without visualization): {movie_error}"
            )
            import traceback
        ...
        print(
            f"⚠ Warning: 3D visualization failed (continuing without visualization): {e}"
        )
        import traceback
...
def _visualize_3d_vessels(img: vtkImageData, viz_dir: Path) -> None:
    ...
    try:
        ...
        print(
            "Creating 3D vessel visualization (VESSELS ONLY - no fibroglandular or background)..."
        )
        ...
        is_probability = (
            data_max <= 1.0
            and data_min >= 0.0
            and not np.allclose(data_3d, data_3d.astype(bool))
        )
        ...
        if is_probability:
            print(
                "  Detected probability data - using volume rendering with probability values (notebook style)"
            )
        else:
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                print("  1) Launch the script under a virtual framebuffer:")
                print(
                    "       xvfb-run -s '-screen 0 1920x1080x24' python 3dslicer_v05_optimized_island_connection.py ..."
                )
                print("  2) Install vtk-osmesa to enable pure offscreen rendering:")
                print("     conda install -c conda-forge vtk-osmesa")
                print("  3) Use --no-visualizations to skip all visualizations")
...
def _visualize_intermediate_stage(
    ...
    non_zero_voxels = np.count_nonzero(visualization_data > VISUALIZATION_THRESHOLD)
    print(
        f"Non-zero voxels: {non_zero_voxels} ({100*non_zero_voxels/visualization_data.size:.2f}% of volume)"
    )
    ...
    try:
        ...
        try:
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                file_size = output_path.stat().st_size / (1024 * 1024)
                print(
                    f"✓ {stage.title()} stage visualization saved: {output_path} ({file_size:.2f} MB)"
                )
            else:
                print("⚠ Warning: MP4 file created but appears empty or missing")
        except Exception as movie_error:
    ...
    try:
        ...
        try:
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                file_size = output_path.stat().st_size / (1024 * 1024)
                print(
                    f"✓ Centerlines visualization saved: {output_path} ({file_size:.2f} MB)"
                )
            else:
                print("⚠ Warning: MP4 file created but appears empty or missing")
        except Exception as movie_error:
...
def _extract_skeleton_centerlines(
    ...
    binary_data = (data > 0).astype(np.uint8).reshape((dims[2], dims[1], dims[0]))
...
def _extract_vmtk_centerlines(
    ...
    if scalars:
        data = vtknp.vtk_to_numpy(scalars)
        non_zero = np.count_nonzero(data > THRESHOLD_FOR_COUNTING)
        total = data.size
        print(
            f"  Input data: {non_zero}/{total} voxels above threshold ({100*non_zero/total:.2f}%)"
        )
        if non_zero == 0:
    ...
    print(
        f"  Surface generated: {surface.GetNumberOfPoints()} points, {surface.GetNumberOfCells()} cells"
    )
    ...
    print(
        f"  Decimated surface: {decimated.GetNumberOfPoints()} points, {decimated.GetNumberOfCells()} cells"
    )
    ...
    try:
        ...
        network = net.Network
        print(
            f"  Network extracted: {network.GetNumberOfPoints()} points, {network.GetNumberOfCells()} cells"
        )
    ...
    *,
    ...
    if ipath.suffix.lower() == ".npy":
        ...
        if data.ndim == NUMPY_4D_DIMENSION:
            ...
            if channel >= data.shape[0]:
                raise ValueError(
                    f"Channel {channel} not available. Input has {data.shape[0]} channels."
                )
        ...
        if is_probability:
            print(
                f"Detected probability data (range: [{data_min:.4f}, {data_max:.4f}])"
            )
            print(
                f"Non-zero voxels: {np.count_nonzero(data)} ({100*np.count_nonzero(data)/data.size:.2f}% of volume)"
            )
            ...
            print(
                f"  Binarizing for centerline extraction (threshold={centerline_threshold}, matching visible vessels)"
            )
            print(
                f"  This captures {voxels_above_threshold} voxels ({voxels_above_threshold/data.size*100:.2f}% of volume)"
            )
            print(
                f"  (Using > 0 would give {np.count_nonzero(data > 0)} voxels, which is too slow)"
            )
        ...
        if len(unique_values) > MULTI_LABEL_THRESHOLD:
            ...
            if extract_label is not None:
                if extract_label not in unique_values:
                    raise ValueError(
                        f"Label {extract_label} not found in NRRD. Available labels: {unique_values}"
                    )
                ...
                for key in header.keys():
                    if "Segment" in key and "Name" in key:
                        if "vessel" in header[key].lower() or "vessel" in key.lower():
                            ...
                            print(
                                f"Found vessel segment: {header[key]} (label {vessel_label})"
                            )
                            break

                if vessel_label is None:
                    # Default: use label 2 if it exists (common for vessel segmentations)
                    if DEFAULT_VESSEL_LABEL in unique_values:
                        vessel_label = DEFAULT_VESSEL_LABEL
                        print("Using label 2 as vessels (default)")
                    else:
            ...
            print(
                f"Extracted label {target_label}: {np.count_nonzero(data)} voxels ({100*np.count_nonzero(data)/data.size:.2f}% of volume)"
            )
        else:
    ...
    if enable_visualizations and viz_dir:
        try:
            ...
            print(
                "  Note: On compute nodes, visualizations may fail due to rendering limitations."
            )
            print(
                "  The script will continue with centerline extraction even if visualization fails."
            )
            ...
            print(
                f"⚠ Warning: Initial visualization failed (continuing with centerline extraction): {e}"
            )
    ...
    if method == "vessel_preprocessing":
                ...
                try:
                    ...
                    if scalars:
                        ...
                        _visualize_intermediate_stage(
                            ...
                            viz_dir,
                            "preprocessed",
                            "Vessel preprocessing (standard method)",
                        )
                except (SystemExit, KeyboardInterrupt):
                    raise
                except Exception as e:
                    print(
                        f"⚠ Warning: Preprocessing visualization failed (continuing with centerline extraction): {e}"
                    )
        ...
        if enable_visualizations and viz_dir:
            try:
                ...
                print(
                    f"⚠ Warning: Skeletonization visualization failed (continuing with centerline extraction): {e}"
                )
    ...
    if enable_visualizations and viz_dir:
        try:
            ...
            print(
                f"⚠ Warning: Centerlines visualization failed (centerline extraction completed successfully): {e}"
            )
    ...
    if num_points > 0:
        ...
        if enable_visualizations and viz_dir:
            ...
            if method == "vessel_preprocessing" and use_island_connection:
                mp4_files.append(
                    (
                        "2. Skeleton (Before Connection)",
                        "skeleton_before_connection_stage_visualization.mp4",
                    )
                )
                mp4_files.append(
                    (
                        "3. Skeleton (After Connection)",
                        "skeleton_after_connection_stage_visualization.mp4",
                    )
                )
                mp4_files.append(
                    ("4. Dilated Skeleton", "skeleton_dilated_stage_visualization.mp4")
                )
            elif method == "vessel_preprocessing":
                mp4_files.append(
                    ("2. Preprocessed Data", "preprocessed_stage_visualization.mp4")
                )
            elif method == "skeletonization":
                mp4_files.append(
                    ("2. Skeletonization", "skeletonization_stage_visualization.mp4")
                )

            # Final centerlines
            mp4_files.append(
                (f"{len(mp4_files) + 1}. Final Centerlines", "final_centerlines_3d.mp4")
            )
        ...
        print("\nTroubleshooting:")
        print(
            "  - Check the intermediate visualizations to see if preprocessing worked"
        )
