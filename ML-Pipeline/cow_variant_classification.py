...
def parse_args():
    ...
    ap.add_argument("--data-dir", type=Path, required=True, help="Folder with .vtp/.vtk (searched recursively)")
    ap.add_argument("--outdir", type=Path, required=True, help="Output folder (features.csv, preview)")
    ap.add_argument("--labels", type=Path, required=False, help="Optional: CSV labels file OR directory of per-case JSONs")
    ap.add_argument("--id-column", default="case_id", help="ID column name in labels file")
    ap.add_argument("--label-column", required=False, help="Binary label column (e.g., fetal_pca_variant)")
    ap.add_argument("--id-regex", default=r"^([A-Za-z0-9_-]+)", help="Regex to extract ID from filename stem")
    ap.add_argument("--model", choices=["rf", "lr"], default="rf", help="Baseline model: RandomForest (rf) or LogisticRegression (lr)")
    ap.add_argument("--test-size", type=float, default=0.2, help="Test split fraction")
    ap.add_argument("--val-size", type=float, default=0.1, help="Validation fraction (of the whole dataset)")
    ap.add_argument("--random-state", type=int, default=42, help="Random seed")
    return ap.parse_args()

def build_labels_from_cow_variant_json(variants_dir: Path, out_csv: Path) -> pd.DataFrame:
    rows = []
    for p in sorted(Path(variants_dir).glob("*.json")):
        ...
        for group, sub in obj.items():
            if isinstance(sub, dict): # only handle sub-dictionaries (each with segment names → True/False)
                for k, v in sub.items():
                    col = re.sub(r'[^A-Za-z0-9_]', '_', f"{group}_{k}") # make clean column name: i.e. "fetal_L_PCA" or "anterior_Acom"
                    if isinstance(v, (bool, np.bool_)): # store True/False as integer 1/0
                        flat[col] = int(bool(v))
                    else:
                        flat[col] = v
        rows.append(flat)
    ...
    if "fetal_L_PCA" in df.columns and "fetal_R_PCA" in df.columns:
        df["fetal_pca_variant"] = ((df["fetal_L_PCA"] == 1) | (df["fetal_R_PCA"] == 1)).astype(int)
...
def find_binary_label_columns(df: pd.DataFrame) -> list[str]:
    ...
    for c in df.columns:
        if c == "case_id": 
            continue
        vals = set(pd.Series(df[c]).dropna().astype(float).astype(int).unique().tolist())
...
def poly_to_row(poly, file: Path) -> Dict[str, float]:
    ...
    row.update({
        ...
        "n_points": float(poly.n_points), "n_cells": float(poly.n_cells),
    })
...
def load_labels(path: Path, id_col: str, label_col: str) -> pd.DataFrame:
    if path.suffix.lower() == ".csv":
        ...
        df = pd.DataFrame(obj) if isinstance(obj, list) else pd.DataFrame.from_dict(obj, orient="index").reset_index().rename(columns={"index": id_col})
    m = {"true": 1, "false": 0, "yes": 1, "no": 0}
    df[label_col] = pd.Series(df[label_col]).map(lambda v: m.get(str(v).lower(), v)).astype(int)
...
def engineer_features(in_csv: Path, out_csv: Path):
    ...
    # drop geometry
    drop_cols = ["source_file", "bbox_x", "bbox_y", "bbox_z",
                 "bbox_volume", "n_points", "n_cells"]
    df = df.drop(columns=[c for c in drop_cols if c in df.columns])

    # keep only morphometric-type features
    morpho_cols = [c for c in df.columns
                   if any(k in c.lower() for k in
                          ["radius", "length", "tortuosity",
                           "curvature", "degree", "labels"])]
...
def train_baseline(feats_engineered_csv: Path,
                   labels_source: Path,
                   id_col: str,
                   label_col: str,
                   outdir: Path,
