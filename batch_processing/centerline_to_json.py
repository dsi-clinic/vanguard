...
def _calculate_tortuosity(points: np.ndarray) -> float:
    ...
    if euclidean_dist < 1e-6:  # noqa: PLR2004
        return 0.0
...
def _estimate_radius_from_segmentation(
    ...
    for point in centerline_points:
        ...
        # Check bounds
        if (
            0 <= voxel_z < segmentation.shape[0]
            and 0 <= voxel_y < segmentation.shape[1]
            and 0 <= voxel_x < segmentation.shape[2]
        ):
...
def _find_bifurcations(
    centerlines: vtkPolyData, point_tolerance: float = 2.0
) -> list[dict]:
    ...
    while lines.GetNextCell(id_list):
        ...
        if n_cell_points < 2:  # noqa: PLR2004
            continue
    ...
    for pt_id, segments in point_to_segments.items():
        if len(segments) >= 3 and pt_id not in processed_points:  # noqa: PLR2004
            ...
            if len(all_segments) >= 3:  # noqa: PLR2004
                # Calculate midpoint of bifurcation region
                region_coords = points[region_points]
                midpoint_idx = region_points[
                    np.argmin(cdist([point_coord], region_coords)[0])
                ]
                ...
                bifurcations.append(
                    {
                        ...
                        "points_angle": region_points[
                            :3
                        ],  # First 3 points for angle calculation
...
def _calculate_bifurcation_angles(
    bifurcation: dict, centerlines: vtkPolyData, points: np.ndarray
) -> dict[str, float]:
    ...
    angles = {}
    if len(segment_vectors) >= 3:  # noqa: PLR2004
        v1, v2, v3 = segment_vectors[0], segment_vectors[1], segment_vectors[2]
        if v1 is not None and v2 is not None:
            angles["seg1/seg2"] = float(
                np.arccos(np.clip(np.dot(v1, v2), -1, 1)) * 180 / np.pi
            )
        if v1 is not None and v3 is not None:
            angles["seg1/seg3"] = float(
                np.arccos(np.clip(np.dot(v1, v3), -1, 1)) * 180 / np.pi
            )
        if v2 is not None and v3 is not None:
            angles["seg2/seg3"] = float(
                np.arccos(np.clip(np.dot(v2, v3), -1, 1)) * 180 / np.pi
            )
...
def centerlines_to_json(
    ...
    if segmentation_path:
        ...
        if seg_data is not None:
            ...
            segmentation = (seg_data > 0.5).astype(bool)  # noqa: PLR2004
    ...
    for seg in segments:
        ...
        if segmentation is not None:
            radii = _estimate_radius_from_segmentation(
                seg_points, segmentation, spacing
            )
    ...
    for bif in bifurcations:
        ...
        if bif["segments"]:
            vessel_key = str(
                bif["segments"][0] + 1
            )  # +1 because segments are 0-indexed
            if vessel_key not in result:
                result[vessel_key] = {}
            ...
            if segmentation is not None and bif["midpoint"] < len(points):
                ...
                radii = _estimate_radius_from_segmentation(
                    [point], segmentation, spacing
                )
                # midpoint_radius could be used for future enhancements
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert VTK centerlines to JSON format"
    )
    parser.add_argument("centerline", help="Input VTK PolyData file (.vtp or .vtk)")
    parser.add_argument("output", help="Output JSON file")
    parser.add_argument(
        "--segmentation", help="Original segmentation file for radius estimation"
    )
    parser.add_argument(
        "--spacing",
        nargs=3,
    ...
    result = centerlines_to_json(
        args.centerline, args.segmentation, tuple(args.spacing)
    )
