#!/usr/bin/env python3
"""Reduce per-study legacy-vs-primary benchmark records into summary artifacts."""
...
DEFAULT_BENCHMARK_DIR = Path("/net/projects2/vanguard/benchmarks/legacy_vs_primary")
...
def build_pairwise_comparison(records: list[dict[str, Any]]) -> dict[str, int]:
    """Count primary-vs-legacy wins based on voxel counts."""
    primary_better = 0
    legacy_better = 0
    tied = 0
    comparable = 0
    ...
    for record in records:
        ...
        if not isinstance(legacy_voxels, int | float):
            continue
        if not isinstance(primary_voxels, int | float):
            continue

        comparable += 1
        if primary_voxels > legacy_voxels:
            primary_better += 1
        elif primary_voxels < legacy_voxels:
            legacy_better += 1
        else:
            tied += 1
    ...
    return {
        ...
        "primary_voxels_greater": primary_better,
        "legacy_voxels_greater": legacy_better,
        "voxels_tied": tied,
    }
