...
def convert_breast_npy_to_nifti(npy_path, output_path, spacing=(1.0, 1.0, 1.0)):
    ...
    print(f"  Non-zero percentage: {100 * np.count_nonzero(binary_data) / binary_data.size:.2f}%")

def main():
    parser = argparse.ArgumentParser(description="Convert breast tissue .npy files to .nii.gz")
    ...
    for i, npy_file in enumerate(npy_files[:args.max_files]):
        output_file = output_dir / f"{npy_file.stem}.nii.gz"
        try:
            convert_breast_npy_to_nifti(npy_file, output_file, args.spacing)
