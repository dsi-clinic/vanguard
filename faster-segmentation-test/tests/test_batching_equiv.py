...
def _write_subject(image_dir: Path, add_dir: Path, name: str, shape, rng):
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pred_and_save_masks_3d_divided(
            TinyDVNet(), str(ckpt_path), make_dataset(), 3, str(out_orig),
            num_workers=0,
        )
        ...
        predict_fast.predict_vessel_batched(
            unet, make_dataset(), n_classes=3, save_masks_dir=str(out_fast),
            device=device, batch_size=7, num_workers=0, use_amp=False,
        ...
        for name in ("subjA", "subjB"):
            ...
            print(f"{name}: shape={a.shape} bit_identical={identical} max_abs_diff={max_abs:g}")
            ok = ok and identical
