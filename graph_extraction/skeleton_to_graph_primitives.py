...
def extract_segments(graph: nx.Graph) -> list[PathList]:
    ...
    visited_edges: set[Segment] = set()

    for node in graph.nodes():
        if graph.degree(node) != SEGMENT_DEGREE:
            for nei in graph.neighbors(node):
                edge_key = _canonical_edge(node, nei)
                ...
                while True:
                    path.append(curr)
                    visited_edges.add(_canonical_edge(prev, curr))
                    if graph.degree(curr) != SEGMENT_DEGREE:
                        break
        ...
        if graph.degree(node) >= BIFURCATION_DEGREE:
            ...
            for i in range(len(vecs)):
                for j in range(i + 1, len(vecs)):
                    angles[f"pair{pair_idx}"] = angle(vecs[i], vecs[j])
                    pair_idx += 1
...
def build_vessel_json(
    ...
    bifurcations: list[dict[str, Any]],
    segment_metrics_by_path: dict[PathKey, dict[str, Any]] | None = None,
    ...
    for idx in vessel_labels:
        ...
        for path in segs:
            if segment_metrics_by_path is None:
                seg_metrics.append(compute_segment_metrics(path, radius_map))
                continue
            path_key: PathKey = tuple(path)
            metric = segment_metrics_by_path.get(path_key)
            if metric is None:
                metric = compute_segment_metrics(path, radius_map)
...
def _canonical_edge(a: Point3D, b: Point3D) -> Segment:
    """Return a stable undirected edge key for deduplication."""
