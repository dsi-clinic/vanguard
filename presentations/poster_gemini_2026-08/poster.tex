...
\logoright{\includegraphics[height=6.9cm]{logos/uw_madison_white.pdf}}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Introduction + Motivation}
    ...
    However, only 30\% of patients achieve a pathologic complete response (pCR)
  ...
  \begin{block}{Ultrafast MRI Dataset}
    ...
    The dataset used in this study consists of ultrafast breast MRI scans from
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Prediction Pipeline}
   ...
   Vessel segmentation is performed using a \textbf{nnU-Net model}, and voxels are
   given a binary mask indicating whether they are part of a vessel or not. 
   Then the segmentation is simplified into a one-voxel wide network of vessels
   using the \textbf{tc4d} algorithm. The simplified network is then converted into a
   graph using one of the three methods described above. Finally, a \textbf{Graph Neural 
   Network (GNN)} is trained to predict treatment response from the graph representation 
   of the vascular network.

  \end{block}

  \begin{block}{Graph Neural Network Architecture}

   After the graph is constructed, we use a GNN to predict treatment response from the 
   graph representation of the vascular network. The GNN consists of several layers of
   \textbf{graph convolutional networks (GCNs)} followed by a global pooling layer and a fully
   connected layer. The GCN layers learn to aggregate information from neighboring 
   nodes in the graph, while the global pooling layer aggregates information from 
   all nodes in the graph to produce a fixed-size representation of the entire graph. 
   Finally, the fully connected layer maps the graph representation to a binary
   prediction of treatment response (pCR vs. non-pCR). 

\vspace{1em}

% Vector PDF (25.3 x 9.1 cm, no raster layers), so it scales cleanly. At full
% column width it renders 36 x 12.9 cm, i.e. its text scales up ~1.4x. Spanning
% the full column matters: in a 0.4\textwidth sub-column the labels shrink to
% roughly 40% of the size drawn.
\begin{figure}
  \includegraphics[width=\linewidth]{figures/GNN_arch.pdf}
  \caption{GNN architecture: 4D DCE-MRI through the vessel graph to a pCR probability.}
\end{figure}
