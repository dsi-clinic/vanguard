...
- Saves to radiomics/figures/{pid}_dce_tumor_zoom.png
"""
...
def bbox_2d_from_mask(mask2d: np.ndarray) -> tuple[int, int, int, int]:
    """Compute a tight 2D bounding box (ymin, ymax, xmin, xmax) from a mask slice."""
...
def expand_bbox(
    ...
    """Expand a 2D bbox by a factor while clamping to image size (H, W)."""
    cy = 0.5 * (ymin + ymax)
    cx = 0.5 * (xmin + xmax)
...
def build_phase_paths(images_dir: Path, pid: str) -> list[Path]:
    """Collect existing phase paths {pid}/{pid}_0000..0005.nii.gz for a patient."""
    paths: list[Path] = []
...
def parse_phase_number(path: Path) -> int | None:
    """Parse the 4-digit phase index from a phase filename, if present."""
    m = _phase_re.search(path.name)
    return int(m.group(1)) if m else None


def main() -> None:
    """Parse CLI arguments and generate the DCE tumor zoom figure."""
    ...
    ap.add_argument(
        "--outdir",
        type=Path,
        default=Path("radiomics/figures"),
    )
    args = ap.parse_args()
    ...
    fig.suptitle(
        f"{args.pid} — DCE phases with tumor mask (zoom={args.zoom_factor}×)",
        y=0.98,
    )
