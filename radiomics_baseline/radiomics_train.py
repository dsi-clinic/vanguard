...
def load_labels(labels_csv: str) -> pd.DataFrame:
    ...
    assert "pcr" in lab.columns, "labels.csv must contain column 'pcr'"
    return lab
...
def corr_prune(X: pd.DataFrame, thr: float) -> Tuple[pd.DataFrame, list]:
    """Drop one of any pair of features with |rho| >= thr (train-only)."""
    if thr <= 0 or thr >= 1 or X.shape[1] < 2:
        return X, []
    ...
    print(f"[DEBUG] corr-prune @ {thr:.2f}: dropped={len(drop_cols)}, kept={X_kept.shape[1]}")
...
def apply_kbest(
    ...
    if Xtr_for_kbest.isna().values.any():
        col_medians = Xtr_for_kbest.median(axis=0)
        Xtr_for_kbest = Xtr_for_kbest.fillna(col_medians)
        print("[DEBUG] apply_kbest: filled NaNs in Xtr with column medians for k-best selection.")
...
# Plotting
# ---------------------------
def plot_roc(y_true, y_prob, outpath: Path) -> None:
...
def plot_pr(y_true, y_prob, outpath: Path) -> None:
    if len(np.unique(y_true)) < 2:
        return
...
def plot_calib(y_true, y_prob, outpath: Path) -> None:
    if len(np.unique(y_true)) < 2:
        return
    prob_true, prob_pred = calibration_curve(
        y_true, y_prob, n_bins=min(10, len(y_true)), strategy="quantile"
    )
    plt.figure()
...
def main() -> None:
    ...
    Xtr, dropped = corr_prune(Xtr, args.corr_threshold)
    Xte = Xte.reindex(columns=Xtr.columns, fill_value=np.nan)
    ...
    if args.cv_folds and args.cv_folds > 1:
        ...
        auc_cv = (
            float(roc_auc_score(ytr, p_cv)) if len(np.unique(ytr)) > 1 else float("nan")
        )
        ...
        if subtype_col is not None:
            ...
            for sub_val in sorted(sub_tr.dropna().unique()):
                mask = sub_tr == sub_val
                y_true_sub = ytr[mask.values]
                y_prob_sub = p_cv[mask.values]
    ...
    auc_test_by_subtype = None
    if subtype_col is not None:
        sub_te = labels.loc[Xte.index, subtype_col]
        auc_test_by_subtype = {}
        for sub_val in sorted(sub_te.dropna().unique()):
            mask = sub_te == sub_val
            y_true_sub = yte[mask.values]
            y_prob_sub = p_te[mask.values]
            if len(np.unique(y_true_sub)) < 2:
                auc_sub = float("nan")
            else:
                auc_sub = float(roc_auc_score(y_true_sub, y_prob_sub))
            auc_test_by_subtype[str(sub_val)] = auc_sub
